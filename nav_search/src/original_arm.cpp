...
public:
  ...
  : Node("arm_search")
  {
    ...
    start_angle_     = this->declare_parameter<double>("start_angle", -3.14);
    end_angle_       = this->declare_parameter<double>("end_angle",   3.14);
 ...
 void init_move_group_if_needed()
{
  ...
  // Node used only for the parameter client
  ...
  // Request parameters asynchronously
  auto future = param_client->get_parameters(
    {"robot_description", "robot_description_semantic"});

  // We MUST spin an executor so the service response actually arrives
  ...
  // Declare on this node before setting
  ...
  // Copy them onto THIS node
  this->set_parameters(params);

  RCLCPP_INFO(get_logger(), "Initializing MoveGroupInterface...");

  move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(
    shared_from_this(),   // this node
    planning_group_       // e.g. "interbotix_arm"
  );
 ...
 void execute_scan(const std::shared_ptr<ScanGoalHandle> goal_handle)
{
  ...
  result->y_base = 0.0f;
    ...
    for (int i = 0; i < steps; ++i) {
      ...
      feedback->progress      = static_cast<float>(100.0 * (static_cast<double>(i) / (steps - 1)));
      ...
      RCLCPP_INFO(get_logger(), "Moving base joint '%s' to angle %.3f rad",
                  base_joint_name_.c_str(), angle);
