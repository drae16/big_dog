...
public:
  ...
  : Node("arm_search_tracking")
  {
    ...
    camera_height_        = this->declare_parameter<double>("camera_height",       0.25);  // z = b
    radius_min_           = this->declare_parameter<double>("camera_radius_min",   0.20);
    radius_max_           = this->declare_parameter<double>("camera_radius_max",   0.35);
    radius_scale_factor_  = this->declare_parameter<double>("camera_radius_scale", 0.2);

    desired_dist_         = this->declare_parameter<double>("desired_distance", 0.75);
    desired_dist_tol_     = this->declare_parameter<double>("desired_distance_tolerance", 0.10);
    

    side_offset_ = this->declare_parameter<double>("side_offset", 0.35);
...
private:
    ...
    const geometry_msgs::msg::Point &target_arm)
  {
   ...
   //y axis is always parallel to the ground
    ...
    tf2::Quaternion q;
    R.getRotation(q);
    tf2::Vector3 ex(1,0,0);
    tf2::Vector3 ex_rot = tf2::quatRotate(q, ex);
    RCLCPP_INFO(get_logger(), "quat rotates +X to: %.2f %.2f %.2f",
            ex_rot.x(), ex_rot.y(), ex_rot.z());
    q.normalize();
  ...
  bool move_wrist_to(double target_angle)
{
    ...
    double dist_tolerance)
  {
    ...
    // Wait for goal handle, tracking while we wait
    while (future_goal.wait_for(100ms) == std::future_status::timeout) {
      // Track target while waiting for goal acceptance
    ...
    // Track while Nav2 is driving
    while (result_future.wait_for(200ms) == std::future_status::timeout) {
      track_target_once(target_x_map, target_y_map);
    }
  ...
  bool rotate_to_yaw_side_on(
    double target_x_map,
    double target_y_map,
    double yaw_tolerance_rad = 5.0 * M_PI / 180.0,   // ~5 deg
    double ang_speed = 0.5                            // rad/s
  )
  {
    if (!cmd_vel_pub_) {
      RCLCPP_WARN(get_logger(), "cmd_vel publisher not initialized for Phase B rotation");
      return false;
    }
    ...
    double dx = target_x_map - rx;
    double dy = target_y_map - ry;
    double theta = std::atan2(dy, dx);  // angle to target

    // Put target on the LEFT side of the dog (90° ccw from line-of-sight)
    double yaw_goal = normalize_angle(theta + M_PI_2);
    ...
    while (rclcpp::ok()) {
      ...
      if (std::fabs(err) < yaw_tolerance_rad) {
      ...
      // keep tracking target while rotating
      track_target_once(target_x_map, target_y_map);
  ...
  bool drive_forward_to_side_offset(
    double target_x_map,
    double target_y_map,
    double forward_speed = 0.3    // m/s
  )
  {
    if (!cmd_vel_pub_) {
    ...
    // We want to end with roughly side_offset_ distance to the target,
    // so we move forward L where L = sqrt(dist^2 - side_offset_^2).
    double dist_sq = dist * dist;
    double side_sq = side_offset_ * side_offset_;
    if (dist_sq <= side_sq) {
      RCLCPP_WARN(get_logger(),
                  "[Phase B] cannot compute forward distance: dist^2 <= side^2");
      return false;
    }

    double forward_dist = std::sqrt(dist_sq - side_sq);
    RCLCPP_INFO(get_logger(),
                "[Phase B] drive: dist=%.3f, side=%.3f, forward_dist=%.3f",
                dist, side_offset_, forward_dist);
    ...
    // We track traveled distance using odometry in map frame
    while (rclcpp::ok() && traveled < forward_dist) {
      cmd_vel_pub_->publish(cmd);

      // Track target while moving forward
      track_target_once(target_x_map, target_y_map);

      double rx, ry, yaw;
      if (!get_robot_pose_map(rx, ry, yaw)) {
        break;
      }
      double dx_curr = rx - rx0;
      double dy_curr = ry - ry0;
      traveled = std::sqrt(dx_curr*dx_curr + dy_curr*dy_curr);
...
void execute_scan(const std::shared_ptr<ScanGoalHandle> goal_handle)
{
  ...
  // 1) Move to Search pose for the initial scan
  ...
  double search_wrist_angle = 1.97;  // whatever worked in earlier tests

  const int    max_levels  = 3;
  const double delta_rad   = 10.0 * M_PI / 180.0;   // 10 degrees
  ...
  const int max_nav_loops = 5;

  for (int loop = 0; loop < max_nav_loops; ++loop) {
    double rx, ry, ryaw;
    if (!get_robot_pose_map(rx, ry, ryaw)) {
      break;
    }
    ...
    // Re-acquire target with YOLO; if lost, bail out
    ...
    if (!found_again) {
      ...
      move_arm_to_stow_pose();
      goal_handle->succeed(result);  // result->found remains true from initial detection
      return;
    }
  ...
  // === Phase B: side-on final approach with cmd_vel + tracking =============
  {
    ...
    if (get_robot_pose_map(rx, ry, ryaw)) {
      ...
      double dist = std::sqrt(dx*dx + dy*dy);
      RCLCPP_INFO(get_logger(),
                  "[Phase B] starting; dist to target = %.3f m", dist);
    }

    // Keep tracking while we do Phase B
    track_target_once(target_x_map, target_y_map);

    // 1) Rotate so the target will be roughly on the robot's side
    if (!rotate_to_yaw_side_on(target_x_map, target_y_map)) {
      RCLCPP_WARN(get_logger(), "[Phase B] rotation failed");
    } else {
      // 2) Drive forward so we end up side_offset_ away from the target
      if (!drive_forward_to_side_offset(target_x_map, target_y_map)) {
        RCLCPP_WARN(get_logger(), "[Phase B] forward drive failed");
      }
    }
    ...
    if (!call_detect_target(goal->min_confidence, xb_final, yb_final)) {
      RCLCPP_WARN(get_logger(),
                  "[Phase B] target lost in final pose; arm search may need restart");
    } else {
